(function($) {
    var loaded = function() {
        ...
        $('#region').on('click', function() {
            ...
            /*receiveMessage2 = function(data, callback) {
            ...
            //chrome.runtime.onMessage.addListener(receiveMessage2);
            chrome.tabs.executeScript({
                'file': 'static/js/region2.js' //region2.js
                //'runAt': 'document_idle'
            }, function(res) {
                console.log("RES", res);
            });

        /*chrome.runtime.sendMessage({action:true}, function(response) {
                    console.log("FFF");
                });
            window.close();*/
        });
    ...
    /*if (chrome.extension.sendMessage && port.name != commPortName) {
        ...
        switch (msg.topic)
        {
            ...
            case 'scrollDone':
                ...
                chrome.runtime.sendMessage({action:true}, function() {
                    console.log("FFF");
                });
